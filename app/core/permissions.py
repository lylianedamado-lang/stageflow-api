...
def get_current_user(
    ...
    """Identifie l'appelant a partir du jeton Bearer. Leve 401 si impossible."""
    if not token:
...
def require_roles(*allowed_roles: UserRole) -> Callable[..., User]:
    """Fabrique une dependance qui n'autorise que les roles indiques."""

    def dependency(current_user: CurrentUser) -> User:
        if current_user.role not in allowed_roles:
            attendus = ", ".join(r.value for r in allowed_roles)
            raise PermissionDeniedError(
                f"Role requis : {attendus}. Role actuel : {current_user.role.value}"
            )
        return current_user
