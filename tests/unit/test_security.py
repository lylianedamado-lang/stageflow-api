...
class TestHashing:
    def test_le_hash_ne_contient_pas_le_mot_de_passe(self):
        hashe = hash_password("Passw0rd!")
        assert "Passw0rd!" not in hashe
    ...
    def test_verification_reussit_avec_le_bon_mot_de_passe(self):
        assert verify_password("Passw0rd!", hash_password("Passw0rd!")) is True

    def test_verification_echoue_avec_un_mauvais_mot_de_passe(self):
        assert verify_password("Mauvais", hash_password("Passw0rd!")) is False

    def test_verification_ne_leve_pas_sur_un_hash_invalide(self):
        assert verify_password("Passw0rd!", "pas-un-hash") is False
...
class TestJWT:
    ...
    def test_un_jeton_expire_est_rejete(self):
        ...
        with pytest.raises(AuthenticationError, match="expire"):
